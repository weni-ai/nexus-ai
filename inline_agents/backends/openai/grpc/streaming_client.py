...
# ANSI color codes for gRPC logs
CYAN = "\033[96m"
GREEN = "\033[92m"
RED = "\033[91m"
YELLOW = "\033[93m"
...
# Colored prefix for easy identification
...
class MessageStreamingClient:
    ...
    def _log_grpc_error(self, operation: str, e: grpc.RpcError) -> None:
        ...
        # Common error explanations to help server owners
        error_hints = {
    ...
    def send_setup_message(
        ...
        try:
            ...
            logger.info(f"{GRPC_PREFIX} {GRPC_OK} Setup response | {GREEN}success={response.success}{RESET} | session_id={response.session_id}")
            return {
    ...
    def send_delta_message(
        ...
        logger.info(f"{GRPC_PREFIX} Sending delta | {CYAN}target={self.target}{RESET} | msg_id={msg_id} | content_len={content_len}")
        ...
        try:
            ...
            logger.info(f"{GRPC_PREFIX} {GRPC_OK} Delta sent | msg_id={msg_id} | {GREEN}status={response.status}{RESET}")
            return {
    ...
    def send_completed_message(
        ...
        """
        content_len = len(content) if content else 0
        logger.info(f"{GRPC_PREFIX} Sending completed | {CYAN}target={self.target}{RESET} | msg_id={msg_id} | content_len={content_len}")
        ...
        try:
            response = self.stub.SendMessage(message, timeout=timeout)
            logger.info(f"{GRPC_PREFIX} {GRPC_OK} Completed sent | msg_id={msg_id} | {GREEN}status={response.status}{RESET}")
            return {
                "status": response.status,
                "msg_id": response.msg_id,
                "message": response.message,
        ...
        Example:
            for response in client.stream_messages_with_setup(
                ...
                if response['is_final']:
                    break
        """
        logger.info(f"{GRPC_PREFIX} Starting stream | {CYAN}target={self.target}{RESET} | contact_urn={contact_urn} | msg_id={msg_id}")
    ...
    def send_single_message(
        ...
        try:
            ...
            logger.info(f"{GRPC_PREFIX} {GRPC_OK} Single message sent | msg_id={msg_id} | {GREEN}status={response.status}{RESET}")
            return {
                "status": response.status,
                "msg_id": response.msg_id,
                "message": response.message,
