...
class TestInvokeAgentsAsyncFailurePath(TestCase):
    ...
    def test_get_default_error_message_caches_resolved_default(self, mock_connect_cls):
        ...
        result2 = self.backend._get_default_error_message(uuid)

        self.assertEqual(result1, _TEST_ERROR_MESSAGES["pt-br"])
...
class SetOpenAIClientTestCase(TestCase):
    """Client resolution per model vendor.

    A vendor that resolves to no client leaves the process-wide default in place, which for
    `aws_mantle` means Bedrock model IDs are sent to api.openai.com and rejected.
    """

    def setUp(self):
        self.backend = OpenAIBackend()

    def call(self, credentials, vendor):
        ...
        with (
            ...
            return async_openai, set_client, set_key

    def test_mantle_without_credentials_uses_settings(self):
        async_openai, set_client, _ = self.call({}, "aws_mantle")

        async_openai.assert_called_once_with(
            base_url="https://bedrock-mantle.us-west-2.api.aws/openai/v1",
            api_key="env-bedrock-key",
        )
        set_client.assert_called_once()

    def test_mantle_credentials_win_over_settings(self):
        credentials = {"api_key": "project-key", "api_base": "https://bedrock-mantle.us-east-1.api.aws/openai/v1"}

        async_openai, set_client, _ = self.call(credentials, "aws_mantle")
    ...
    def test_mantle_partial_credentials_fall_back_per_field(self):
        async_openai, _, _ = self.call({"api_key": "project-key"}, "aws_mantle")
    ...
    def test_openai_without_credentials_keeps_default_client(self):
        async_openai, set_client, set_key = self.call({}, "openai")
    ...
    def test_openai_with_base_url_builds_client(self):
        ...
        async_openai, set_client, set_key = self.call(credentials, "openai")
    ...
    def test_openai_without_base_url_sets_only_the_key(self):
        async_openai, set_client, set_key = self.call({"api_key": "k"}, "openai")
    ...
    def test_unsupported_vendor_is_ignored(self):
        ...
        async_openai, set_client, set_key = self.call(credentials, "vertex_ai")
