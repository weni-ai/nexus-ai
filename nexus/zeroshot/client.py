...
class InvokeModel:
    def __init__(
        ...
        region_name: str = settings.ZEROSHOT_BEDROCK_AWS_REGION,
    ) -> None:
    ...
    def _invoke_runpod(self, prompt):
        ...
        response_nlp = requests.post(headers=headers, url=url, data=payload)
    ...
    def _invoke_zeroshot(self, model_backend: str):
        return {"runpod": self._invoke_runpod, "bedrock": self._invoke_bedrock}.get(model_backend)
    ...
    def _invoke_function_calling(self):
        ...
        prediction: str = classifier.predict(
            message=self.zeroshot_data.get("text"), flows=flow_dto_list, language=self.zeroshot_data.get("language")
        )

        formated_prediction = {"output": prediction}
