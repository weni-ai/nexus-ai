...
def _parse_inline_message_uuid_from_key(key: str, project_uuid: str) -> str | None:
    ...
    message_uuid = key[len(prefix) : -len(".jsonl")]
    try:
        UUID(message_uuid)
    except ValueError:
        return None
...
def collect_trace_s3_objects(
    ...
    *,
...
) -> list[tuple[S3ObjectRef, str]]:
    project_uuid = str(project.uuid)
    collected: list[tuple[S3ObjectRef, str]] = []
    seen_keys: set[tuple[str, str]] = set()

    def add_object(*, bucket: str, key: str, source_model: str, source_uuid: str, trace_type: str) -> None:
    ...
    if include_inline:
        for obj in collect_inline_trace_s3_objects(project):
            add_object(
                bucket=obj.bucket,
                key=obj.key,
