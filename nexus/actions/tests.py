...
class SimulationEndpointsTestCase(TestCase):
    def setUp(self):
        self.factory = APIRequestFactory()
    ...
    def test_preview_keeps_legacy_task_kwargs(self, mock_apply_async):
    ...
    @patch("nexus.actions.api.views.start_inline_agents.apply_async")
    def test_simulation_adds_simulation_flag(self, mock_apply_async):
        request = self.factory.post(
    ...
    def test_simulation_manager_model_post_sets_ttl(self, mock_get_redis_write_client):
        redis_client = MagicMock()
        mock_get_redis_write_client.return_value = redis_client
        request = self.factory.post(
        ...
        redis_client.setex.assert_called_once_with(
            f"simulation_manager_model:{self.project.uuid}",
            86400,
            "gpt-4.1",
        )

    @patch("nexus.actions.api.views.get_redis_read_client")
    def test_simulation_manager_model_get_falls_back_to_project_default(self, mock_get_redis_read_client):
        redis_client = MagicMock()
        redis_client.get.return_value = None
        mock_get_redis_read_client.return_value = redis_client
        request = self.factory.get(f"/{self.project.uuid}/simulation/manager-model/")
        force_authenticate(request, user=self.user)

        response = SimulationManagerModelView.as_view()(request, project_uuid=str(self.project.uuid))

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["manager_foundation_model"], self.project.default_supervisor_foundation_model)
    ...
    def test_simulation_end_session_calls_backend(
        self,
        mock_message_factory,
        mock_get_agents_backend,
        mock_get_backend,
    ):
        ...
        mock_get_agents_backend.return_value = "openai"
