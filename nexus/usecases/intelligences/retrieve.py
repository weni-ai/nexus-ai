...
from nexus.intelligences.models import ContentBaseLink, ContentBaseText, Conversation
...
class RetrieveIntelligenceUseCase:
    def get_intelligence(self, intelligence_uuid: str, user_email: str):
...
class RetrieveContentBaseUseCase:
    def get_contentbase(self, contentbase_uuid: str, user_email: str):
        org_use_case = orgs.GetOrgByIntelligenceUseCase()
        user = users.get_by_email(user_email)
    ...
    def get_default_by_project(
        ...
        if not is_superuser:
            user = users.get_by_email(user_email)
            has_project_permission(user=user, project=project, method="GET")

        return get_default_content_base_by_project(project_uuid)


class RetrieveContentBaseTextUseCase:
    def get_contentbasetext(self, contentbasetext_uuid: str, user_email: str):
        org_use_case = orgs.GetOrgByIntelligenceUseCase()
        user = users.get_by_email(user_email)
...
class RetrieveContentBaseFileUseCase:
    def get_contentbasefile(self, contentbasefile_uuid: str, user_email: str):
        ...
        org = org_use_case.get_org_by_contentbasefile_uuid(contentbasefile_uuid)
        has_permission = permissions.can_list_content_bases(user, org)
...
class RetrieveContentBaseLinkUseCase:
    def get_contentbaselink(self, contentbaselink_uuid: str, user_email: str):
        ...
        org = org_use_case.get_org_by_contentbaselink_uuid(contentbaselink_uuid)
        has_permission = permissions.can_list_content_bases(user, org)
    ...
    def get_content_base_link_by_link(self, link: str, content_base_uuid: str):
        links = ContentBaseLink.objects.filter(link=link, content_base__uuid=content_base_uuid)
...
def get_file_info(file_uuid: str) -> Dict:
    try:
        ...
        try:
            ...
            return {"uuid": str(text.uuid), "created_file_name": ".text"}
        except ContentBaseTextDoesNotExist:
            try:
                link = get_by_content_base_link_uuid(file_uuid)
                return {"uuid": str(link.uuid), "created_file_name": f".link:{link.link}"}
            except ContentBaseLinkDoesNotExist:
                return {}


def get_conversation_object(
    project_uuid: str, contact_urn: str, start_date: datetime, end_date: datetime
) -> Conversation:
    project = Project.objects.get(uuid=project_uuid)
    return Conversation.objects.get(
        project=project, contact_urn=contact_urn, start_date__gte=start_date, end_date__lte=end_date
    )
