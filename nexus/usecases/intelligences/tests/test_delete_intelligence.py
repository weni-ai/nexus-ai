...
from nexus.event_domain.recent_activity.mocks import mock_event_manager_notify, mock_message_handler
from nexus.intelligences.models import ContentBase, ContentBaseText, Intelligence
from nexus.usecases.intelligences.tests.mocks import MockFileDataBase

from ..delete import DeleteContentBaseTextUseCase, DeleteContentBaseUseCase, DeleteIntelligenceUseCase
from .intelligence_factory import ContentBaseFactory, ContentBaseTextFactory, IntelligenceFactory


class TestDeleteIntelligenceUseCase(TestCase):
    def setUp(self):
        self.use_case = DeleteIntelligenceUseCase(recent_activity_message=mock_message_handler)
...
class TestDeleteContentBaseUseCase(TestCase):
    ...
    def test_delete_contentbase(self):
        use_case = DeleteContentBaseUseCase(event_manager_notify=mock_event_manager_notify)
