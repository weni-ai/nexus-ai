...
from ..update import UpdateContentBaseTextUseCase, UpdateContentBaseUseCase, UpdateIntelligenceUseCase, UpdateLLMUseCase
from .intelligence_factory import ContentBaseFactory, ContentBaseTextFactory, IntelligenceFactory, LLMFactory
...
class TestUpdateIntelligenceUseCase(TestCase):
    ...
    def test_update_intelligence_name(self):
        ...
        updated_intelligence = self.use_case.update_intelligences(
            intelligence_uuid=self.intelligence.uuid, name=new_name, user_email=self.intelligence.created_by.email
        )
...
class TestUpdateContentBaseUseCase(TestCase):
    ...
    def test_update_contentbase_title(self):
        new_title = "New Title"
        new_language = "pt-br"
        use_case = UpdateContentBaseUseCase(event_manager_notify=mock_event_manager_notify)
...
class TestUpdateContentBaseTextUseCase(TestCase):
    ...
    def test_update_contentbasetext_text(self):
        new_text = "New Text"
        use_case = UpdateContentBaseTextUseCase(event_manager_notify=mock_event_manager_notify)
        updated_contentbasetext = use_case.update_contentbasetext(
            contentbasetext=self.contentbasetext, text=new_text, user_email=self.contentbasetext.created_by.email
        )
...
class TestUpdateLLM(TestCase):
    ...
    def test_update_llm(self):
        new_temperature = 0.5
        new_top_p = 0.5
        setup = {"temperature": new_temperature, "top_p": new_top_p}
        advanced_options = {"test": "test"}
        ...
        updated_llm = usecase.update_llm_by_project(update_llm_dto=update_dto)
