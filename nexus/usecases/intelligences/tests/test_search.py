...
class SearchFileDatabase:
    ...
    def search_data(self, content_base_uuid: str, text: str):
        return {"status": 200, "data": {"response": [{"full_page": "Text1", "filename": "text1.pdf"}]}}
...
class MockGenerativeAIDatabase:
    ...
    answers = {"en": eng_answer, "pt": por_answer, "es": spa_answer}
...
class IntelligenceGenerativeSearchUseCaseTestCase(TestCase):
    ...
    def test_search_eng(self):
        ...
        response = self.usecase.search(
            content_base_uuid=str(self.content_base.uuid), text="text", language=content_base.language
        )
        self.assertEqual(MockGenerativeAIDatabase.eng_answer, response.get("answers")[0])
    ...
    def test_search_spa(self):
        ...
        self.assertEqual(MockGenerativeAIDatabase.spa_answer, response.get("answers")[0])
    ...
    def test_search_por(self):
        ...
        self.assertEqual(MockGenerativeAIDatabase.por_answer, response.get("answers")[0])

    def test_search_base(self):
        content_base = self.content_base
        content_base.language = "en-us"
        content_base.save()
        response = self.usecase.search(content_base_uuid=str(content_base.uuid), text="text", language="base")
        self.assertEqual(MockGenerativeAIDatabase.answers.get("en"), response.get("answers")[0])
