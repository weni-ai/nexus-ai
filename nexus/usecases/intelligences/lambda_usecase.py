...
def create_lambda_conversation(
    ...
    try:
        ...
        message_service.clear_message_cache(
            project_uuid=project_uuid, contact_urn=contact_urn, channel_uuid=payload.get("channel_uuid")
        )
        ...
        try:
            ...
            project_uuid_payload = payload.get("project_uuid")
            if project_uuid_payload:
                integrated_agent = (
                    IntegratedAgent.objects.filter(project__uuid=project_uuid_payload).select_related("agent").first()
                )
        ...
        sentry_context = {"payload": payload, "task_id": task_id, "correlation_id": correlation_id}
        if agent_uuid:
