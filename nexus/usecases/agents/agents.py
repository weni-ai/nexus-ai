...
class AgentUsecase:
    ...
    def assign_agent(self, agent_uuid: str, project_uuid: str, created_by):
        ...
        if str(agent.project.uuid) != project_uuid:
            logging.getLogger(__name__).info("Creating contact fields for agent")
            fields = []
            for contact_field in agent.contact_fields.distinct("key"):
                key = contact_field.key
                value_type = contact_field.value_type
                logging.getLogger(__name__).debug("Contact field", extra={"key": key, "value_type": value_type})
    ...
    def create_agent(self, user: User, agent_dto: AgentDTO, project_uuid: str, alias_name: str = "v1"):
        logging.getLogger(__name__).info("Starting create agent")
    ...
    def create_contact_fields(
        ...
        for contact_field in fields:
            if contact_field.get("key") not in existing_keys:
                ...
                logging.getLogger(__name__).info("Creating contact field", extra={"key": contact_field.get('key'), "value_type": value_type})
    ...
    def create_agent_version(self, agent_external_id, user, agent, team):
        import logging
        logging.getLogger(__name__).info("Creating a new agent version")
    ...
    def create_team_version(self, agent_external_id, user, team):
        import logging
        logging.getLogger(__name__).info("Creating a new team version")
    ...
    def create_skill_alias(
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).info("Created alias", extra={"alias": alias_name, "function": function_name})
            return response
        except self.external_agent_client.lambda_client.exceptions.ResourceConflictException:
            # If alias already exists, update it
            import logging
            logging.getLogger(__name__).info("Alias already exists, updating", extra={"alias": alias_name, "function": function_name})
            ...
            import logging
            logging.getLogger(__name__).error("Error creating/updating alias", extra={"function": function_name, "error": str(e)})
            raise
    ...
    def update_skill(
        ...
        """
        import logging
        logging.getLogger(__name__).info("Starting update Lambda function")
        ...
        import logging
        logging.getLogger(__name__).info("Updated Lambda function")
        ...
        import logging
        logging.getLogger(__name__).info("Updated skill", extra={"skill": skill_object.display_name, "agent": agent.display_name})
    ...
    def handle_agent_skills(self, agent: Agent, skills: List[Dict], files: Dict, user: User, project_uuid: str):
        ...
        for skill in skills:
            ...
            try:
                ...
                import logging
                logging.getLogger(__name__).info("Updating existing skill", extra={"lambda_name": lambda_name})
                ...
                import logging
                logging.getLogger(__name__).info("Creating new skill", extra={"lambda_name": lambda_name})
                self.create_skill(
                    agent_external_id=agent.external_id,
                    file_name=lambda_name,
                    agent_version=agent.current_version.metadata.get("agent_alias_version"),
    ...
    def delete_supervisor_version(self, agent_id: str, version):
        try:
            ...
            import logging
            logging.getLogger(__name__).debug("Deleted supervisor alias", extra={"response_keys": list(response.keys())})
            version.delete()
            return response
    ...
    def _update_supervisor_instructions(self, supervisor_id: str) -> None:
        ...
        import logging
        logging.getLogger(__name__).info("Got supervisor agent")
    ...
    def _insert_human_support_instructions(self, current_instructions: str) -> str | None:
        ...
        if principles_pos == -1:
            import logging
            logging.getLogger(__name__).warning("Could not find principles tag in supervisor instructions")
            return None
    ...
    def _remove_human_support_instructions(self, current_instructions: str) -> str | None:
        ...
        if start_pos == -1:
            import logging
            logging.getLogger(__name__).warning("Could not find human support instructions to remove")
            return None
    ...
    def _add_human_support_actions(self, supervisor_id: str, team: Agent) -> None:
        ...
        import logging
        logging.getLogger(__name__).info("Action groups to process", extra={"count": len(action_groups) if isinstance(action_groups, list) else 1})

        for action_group_id in action_groups:
            import logging
            logging.getLogger(__name__).info("Processing action group", extra={"action_group_id": action_group_id})
            action_data = self._get_action_group_data(human_support_agent_id, action_group_id)
            if not action_data:
                import logging
                logging.getLogger(__name__).warning("Failed to get data for action group", extra={"action_group_id": action_group_id})
                continue
            ...
            if response and "agentActionGroup" in response:
                ...
                import logging
                logging.getLogger(__name__).info("Created action group", extra={"id": created_action_group_id})

        # Update team metadata with created action groups
        if created_action_groups:
            import logging
            logging.getLogger(__name__).info("Saving action groups to team metadata", extra={"count": len(created_action_groups)})
    ...
    def _remove_human_support_actions(self, team: Team) -> None:
        ...
        if not action_groups:
            import logging
            logging.getLogger(__name__).warning("No human support action groups found to remove")
            return
        ...
        for action_group_id in action_groups:
            try:
                import logging
                logging.getLogger(__name__).info("Disabling action group", extra={"action_group_id": action_group_id})
                ...
                import logging
                logging.getLogger(__name__).debug("Function schema for update", extra={"functions": len(function_schema)})
                ...
                import logging
                logging.getLogger(__name__).info("Deleting action group", extra={"action_group_id": action_group_id})
                ...
                import logging
                logging.getLogger(__name__).info("Successfully removed action group", extra={"action_group_id": action_group_id})

            except Exception as e:
                import logging
                logging.getLogger(__name__).error("Error removing action group", extra={"action_group_id": action_group_id, "error": str(e)})
                continue
        ...
        if successfully_removed:
            ...
            team.save(update_fields=["metadata"])
            import logging
            logging.getLogger(__name__).info("Removed action groups from metadata", extra={"count": len(successfully_removed)})
