...
class FlowsClientMock:
    def __init__(self, *args, **kwargs):
        self.list_project_contact_fields = Mock(
            return_value={
                "results": [{"key": "existing_field", "label": "Existing Field", "value_type": "text", "pinned": False}]
            }
        )
...
class TestAgentsUsecase(TestCase):
    def setUp(self):
        ...
        self.usecase = AgentUsecase(
            external_agent_client=BedrockFileDatabaseMock, flows_client=lambda: self.flows_client
        )
    ...
    def test_contact_field_handler(self):
        ...
        self.flows_client.list_project_contact_fields.assert_called_once_with(str(skill_object.agent.project.uuid))

        self.flows_client.create_project_contact_field.assert_called_once_with(
            project_uuid=str(skill_object.agent.project.uuid), key="event", value_type="string"
        )
