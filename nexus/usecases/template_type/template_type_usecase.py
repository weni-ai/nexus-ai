...
class TemplateTypeUseCase:
    def get_intelligences_info_by_project(self, project):
        ...
        for intelligence in org.intelligences.all():
            info["intelligences"].append({"uuid": intelligence.uuid})
        return info
    ...
    def create_template_type(self, template_type_dto: TemplateTypeDTO):  # pragma: no cover
        ...
        try:
            ...
            raise Exception(f"Project `{template_type_dto.project_uuid}` does not exists!") from e
        setup = self.get_setup(project=project)  # Not implemented
        template_type, created = TemplateType.objects.get_or_create(
            uuid=template_type_dto.uuid, defaults=dict(name=template_type_dto.name, setup=setup)
        )
        if not created:
    ...
    def get_by_uuid(self, template_type_uuid: str):
        ...
        except TemplateType.DoesNotExist as e:
            raise Exception(f"Template Type `{template_type_uuid}` does not exists!") from e
        return template_type
