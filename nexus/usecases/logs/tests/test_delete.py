...
class DeleteMessageLogsTestCase(TestCase):
    def create_logs(self):
        with freeze_time(str(pendulum.now().subtract(months=1))):
            message = Message.objects.create(text="Text 1", contact_urn="contact_urn", status="S")
            MessageLog.objects.create(
                message=message,
            )

        with freeze_time(str(pendulum.now().subtract(days=2))):
            message = Message.objects.create(text="Text 2", contact_urn="contact_urn", status="S")
            MessageLog.objects.create(
                message=message,
            )

        message = Message.objects.create(text="Text 3", contact_urn="contact_urn", status="S")
        MessageLog.objects.create(message=message, created_at=pendulum.now())
