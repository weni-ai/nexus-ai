...
class CreateAgentUseCase(ToolsUseCase, InstructionsUseCase):
    ...
    def create_agent(
        ...
        self.handle_tools(agent_obj, project, agent["tools"], files, str(project.uuid), apm_instrumentation)
        ...
        if agent.get("mcps") or agent.get("mcp"):
            ...
            for mcp_item in mcps_data:
                if isinstance(mcp_item, str):
                    ...
                    if mcp:
                        sync_mcp_templates_from_agent_payload(mcp, agent.get("credentials"), agent.get("constants"))
